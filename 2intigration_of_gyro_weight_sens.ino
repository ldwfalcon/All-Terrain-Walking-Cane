  ...
  public:
    void sol() {
      if (redscale.is_ready()) {
        ...
        float yellowreading = yellowscale.get_units()*-1;
        if (redreading > 2 && greenreading > 2 && yellowreading > 2) {
          haslifted = 1;
          solCon.off();
        }
        else if ((redreading > 2 or greenreading > 2 or yellowreading >2) && (redreading < 2 or greenreading < 2 or yellowreading< 2) && haslifted == 1) {
          //can be used by storing data and names in a dictionary and array, then using that to now what solenoids to control.
        
          //one leg on floor scenario
          if (redreading < 2 && yellowreading > 2 && greenreading > 2) {
          ...
          if (greenreading < 2 && yellowreading > 2 && redreading > 2) {
            //Serial.print(realDelay.justFinished());
            if (realDelay.remaining() == 0 && loopcountkeeper == 0) {
              digitalWrite(green, HIGH);
              realDelay.start(1000);
          ...
          if (yellowreading < 2 && redreading > 2 && greenreading > 2) {
            //Serial.print(realDelay.justFinished());
            if (realDelay.remaining() == 0 && loopcountkeeper == 0) {
              digitalWrite(yellow, HIGH);
              realDelay.start(1000);
        ...
        else if (redreading < 2 && greenreading < 2 && yellowreading <2) {
          solCon.off();
